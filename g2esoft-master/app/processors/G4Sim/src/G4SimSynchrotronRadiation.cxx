...
G4SimSynchrotronRadiation::G4SimSynchrotronRadiation(const G4String& processName,
  G4ProcessType type):G4VContinuousDiscreteProcess (processName, type),
...
G4SimSynchrotronRadiation::~G4SimSynchrotronRadiation()
{
    delete genAngle;
}
...
void 
G4SimSynchrotronRadiation::SetAngularGenerator(G4VEmAngularDistribution* p)
{
  if(p != genAngle) {
    delete genAngle;
    genAngle = p;
...
G4bool
G4SimSynchrotronRadiation::IsApplicable(const G4ParticleDefinition& particle)
{
...
G4VParticleChange *
G4SimSynchrotronRadiation::AlongStepDoIt(const G4Track& trackData,
					  const G4Step& stepData )
{
  ...
  if(gamma <= 10  || gamma > 1.0e3 || 0.0 == particleCharge) 
  {
    return G4VContinuousDiscreteProcess::AlongStepDoIt(trackData,stepData);
  }
  ...
  if ( fieldExertsForce )
  {
    ...
    if(perpB > 0.0)
    {
      ...
      G4double beta = sqrt(1-1./(gamma*gamma));
      G4double rho = (aDynamicParticle->GetDefinition()->GetPDGMass()*gamma*beta)/(c_light*perpB);
      G4double energyLoss = 88.46274015906*keV*pow(aDynamicParticle->GetTotalEnergy()/GeV, 4)*beta*c_light*stepData.GetDeltaTime()/(2.*pi*(rho/m)*rho);
      G4double newKinEnergy = kineticEnergy - energyLoss;
...
G4double
G4SimSynchrotronRadiation::GetMeanFreePath(const G4Track& trackData,
					G4double,
  ...
  *condition = NotForced;
  ...
  else
  {
	...
	(2.5*fine_structure_const*c_light); 
      ...
      {
        G4cout << "G4SimSynchrotronRadiation::GetMeanFreePath "
	       << " for particle " 
...
G4VParticleChange* 
G4SimSynchrotronRadiation::PostStepDoIt(const G4Track& trackData,
                                     const G4Step&  stepData      )
...
G4double G4SimSynchrotronRadiation::InvSynFracInt(G4double x)
// direct generation
{
  // from 0 to 0.7
...
G4double G4SimSynchrotronRadiation::GetRandomEnergySR(
         G4double gamma, G4double perpB, G4double mass_c2) 
{
  ...
  { 
    ...
    G4cout << "G4SimSynchrotronRadiation::GetRandomEnergySR :" << '\n' 
	   << std::setprecision(4)
...
void 
G4SimSynchrotronRadiation::BuildPhysicsTable(const G4ParticleDefinition& part)
{
...
void G4SimSynchrotronRadiation::PrintInfoDefinition() 
...
G4double G4SimSynchrotronRadiation::GetContinuousStepLimit(const G4Track&,
							  G4double,
							  G4double,
							  G4double&)
{
...
///////////////////// end of G4SimSynchrotronRadiation.cc
